...
|---------|---------|-------------------|------------|
| **File IPC** | Windows Python | Yes 鈥?AutoCAD LT 2024+ (Windows) | Win32 PrintWindow |
...
- **AutoCAD LT 2024 or newer** 鈥?AutoLISP support was added in LT 2024 for Windows. AutoCAD LT for Mac exists but does **not** support AutoLISP.
- **Python 3.10+** (Windows native 鈥?not WSL Python)
...
- `AUTOCAD_MCP_BACKEND` can be `auto` (default 鈥?tries File IPC, falls back to ezdxf), `file_ipc` (requires AutoCAD), or `ezdxf` (headless only).
...
### `drawing` 鈥?File/drawing management
...
### `entity` 鈥?Entity CRUD + modification
...
### `annotation` 鈥?Text, dimensions, leaders
...
### `pid` 鈥?P&ID operations (CTO symbol library)
...
> P&ID symbol insertion requires the [CAD Tools Online](https://www.cadtoolsonline.com/) (CTO) P&ID Symbol Library installed at `C:\PIDv4-CTO\`. The ezdxf backend has built-in CTO library support. For the File IPC backend, some P&ID operations require additional LISP helpers 鈥?see the P&ID section in the wiki for setup details.

### `view` 鈥?Viewport and screenshot
...
Screenshots use `PrintWindow` (Win32) for the File IPC backend 鈥?works even when AutoCAD is minimized or in the background. The ezdxf backend renders via matplotlib.

### `system` 鈥?Server management

`status`, `health`, `get_backend`, `runtime`, `init`, `execute_lisp`

> `execute_lisp` runs arbitrary AutoLISP code (File IPC only). Pass `data: {code: "(+ 1 2)"}`. This turns the server into an extensible automation platform 鈥?any valid AutoLISP expression can be executed.
...
Python MCP Server (autocad_mcp)
    鈹?
    鈹溾攢鈹€ File IPC Backend 鈹€鈹€鈻?C:/temp/*.json 鈹€鈹€鈻?mcp_dispatch.lsp (AutoCAD LT)
    鈹?  PostMessageW(WM_CHAR) to MDIClient 鈥?no focus steal
    鈹?
    鈹斺攢鈹€ ezdxf Backend 鈹€鈹€鈻?in-memory DXF (headless, no AutoCAD needed)
```

The File IPC backend sends keystrokes to AutoCAD's MDIClient window via `PostMessageW(WM_CHAR)`, triggering the `(c:mcp-dispatch)` AutoLISP command. This approach does **not** steal window focus 鈥?you can continue working in other applications while automation runs.
...
- **`execute_lisp`** 鈥?Run arbitrary AutoLISP code via temp file pattern. Turns the server from a fixed command set into an extensible automation platform.
- **Undo / Redo** 鈥?Single-step undo and redo via `drawing` tool.
- **Drawing open** 鈥?Open existing `.dwg` files programmatically (FILEDIA suppressed).
- **Drawing create** 鈥?Now resets current drawing (erase all + purge) instead of `_.NEW`, preserving the LISP dispatcher namespace.
- **Drawing save with path** 鈥?`save` with a `path` parameter uses SAVEAS; without path uses QSAVE.
- **`get_variables` fix** 鈥?Respects the `names` parameter; returns requested variables with proper type handling.
- **Polyline/leader fix** 鈥?Point arrays properly encoded via semicolon-delimited format.
- **ESC prefix** 鈥?Sends 2x ESC before each dispatch to cancel stale pending commands from prior timeouts.
- **UTF-8/cp1252 fallback** 鈥?Handles non-ASCII characters in LISP result files (AutoCAD writes Windows-1252).
- **Configurable IPC timeout** 鈥?`AUTOCAD_MCP_IPC_TIMEOUT` env var (1鈥?00 seconds, default 10).
- **Thread-safe backend init** 鈥?`asyncio.Lock` prevents parallel initialization races.
...
This repo includes a portable local skill extension for AutoCAD direct drawing workflows.

- `skill-extension/autocad-direct-drawing/` contains the skill bundle
- `USAGE.md` explains the repo layout and setup
- On another machine, set your local AutoCAD and ComfyUI paths in the bridge scripts or environment variables
